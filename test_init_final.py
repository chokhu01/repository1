...
def init():
	...
	basicSetting.append(inputData[15][14:])    #basicSetting[16] : 출력1 셀
	basicSetting.append(inputData[16][14:])    #basicSetting[17] : 출력2 셀
			...
			if basicSetting[11] != "":
				if msg.channel.id == int(basicSetting[11]) : #### 정산채널 채널ID 값넣으면 됨
					...
					if message.content.startswith('!정산 '):
						if basicSetting[10] !="" and basicSetting[12] !="" and basicSetting[14] !="" and basicSetting[15] !="" and basicSetting[16] !="" and basicSetting[17] !=""  :
							...
							embed = discord.Embed(
									description= '```' + SearchID + ' 님의 현재까지 총 루팅 금액은 ' + result2 + ' 이며 분배받을 다이야는 ' + result + ' 다이야 입니다.```',
									color=0xff00ff
									)
